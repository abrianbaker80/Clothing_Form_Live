...
jQuery(document).ready(function ($) {
    ...
    function initializeCategories() {
        ...
        if ($mainCategorySelects.length === 0 && $genderSelects.length === 0) {
            ...
            // IMPROVEMENT: Try to find any select elements for troubleshooting
            console.log('Searching for any select elements in the form...');
            const $allSelects = $('.clothing-submission-form select');
            console.log('Found ' + $allSelects.length + ' total select elements');
            $allSelects.each(function (index) {
                console.log('Select #' + index + ': id=' + $(this).attr('id') + ', name=' + $(this).attr('name') + ', class=' + $(this).attr('class'));
            });
        ...
        // IMPROVEMENT: Immediately initialize gender-based categories if gender selects are found
