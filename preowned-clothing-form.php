...
/**
 * Register GitHub updater admin functions
 * Ensures proper admin menu registration
 */
function preowned_clothing_register_github_updater_admin() {
    // Ensure the admin file is loaded
...
// Use admin_init hook with priority 1 to ensure early loading
add_action('admin_init', 'preowned_clothing_register_github_updater_admin', 1);

/**
 * Register GitHub updater menu item
 * This ensures the settings page is accessible in admin
 */
function preowned_clothing_add_github_updater_menu() {
    // First make sure the admin file is loaded
    $admin_file = plugin_dir_path(__FILE__) . 'includes/github-updater-admin.php';
    if (!file_exists($admin_file)) {
        // Log error if in debug mode
        if (defined('WP_DEBUG') && WP_DEBUG) {
            error_log('GitHub updater admin file not found: ' . $admin_file);
        }
        return;
    }
