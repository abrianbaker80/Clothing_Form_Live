...
if (isset($_SESSION['submission_status'])) {
    ...
    $submission_message = $submission_status === 'success' ?
        'Your clothing item submission was successful! Thank you. Someone from our team will be reaching out to you within 24-48 hours.' : (isset($_GET['submission_message']) ? sanitize_text_field($_GET['submission_message']) : null);
    $submission_message = $submission_status === 'success' ?
        'Your clothing item submission was successful! Thank you. Someone from our team will be reaching out to you within 24-48 hours.' : (isset($_GET['submission_message']) ? sanitize_text_field($_GET['submission_message']) : null);
...
function preowned_clothing_display_form($atts = [])
{
    ...
    // CRITICAL FIX: Explicitly add categories and sizes to options array
    global $clothing_categories_hierarchical, $clothing_sizes;
    $options['categories'] = $clothing_categories_hierarchical;
    $options['sizes'] = $clothing_sizes;

    // Debug - print full options being passed to renderer
    if (defined('WP_DEBUG') && WP_DEBUG) {
        error_log('Form Renderer Options with Categories: ' . print_r($options, true));
    }
