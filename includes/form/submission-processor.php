 ...
 * Handles database operations and transaction management for form submissions
 */
    ...
    public function process($data)
    {
        ...
        try {
            ...
            if (isset($image_result) && isset($image_result['uploaded_files'])) {
                $this->uploader->cleanup_files($image_result['uploaded_files']);
            }
    ...
    private function ensure_tables_exist()
    {
        ...
        if (!$submissions_exists || !$items_exists) {
            if (function_exists('preowned_clothing_check_table')) {
                $created = preowned_clothing_check_table();
                if (!$created) {
